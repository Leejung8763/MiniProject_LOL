    ...
    while maxRetry < 60:
        try:
            ...
            # Error Occured
                ...
                print(f"challengerLeague / {errorTable[challengerListRequest.status_code]['message']} / {errorTable[challengerListRequest.status_code]['description']} / Try after {backoff}")
                time.sleep(backoff)
            else:
                challengerListDto = pd.DataFrame()
                print(f"challengerLeague / {errorTable[challengerListRequest.status_code]['message']} / {errorTable[challengerListRequest.status_code]['description']}")
                break   
        except requests.exceptions.ConnectionError as errc:
            time.sleep(60)
            print(f"challengerLeague / {errc} / {datetime.datetime.now()}")
            maxRetry += 1
    ...
    while maxRetry < 60:
        try:
            ...
            # Error Occured
                ...
                print(f"grandmasterLeague / {errorTable[grandmasterListRequest.status_code]['message']} / {errorTable[grandmasterListRequest.status_code]['description']} / Try after {backoff}")
                time.sleep(backoff)
            else:
                grandmasterListDto = pd.DataFrame()
                print(f"grandmasterLeague / {errorTable[grandmasterListRequest.status_code]['message']} / {errorTable[grandmasterListRequest.status_code]['description']}")
                break
        except requests.exceptions.ConnectionError as errc:
            time.sleep(60)
            print(f"grandmasterLeague / {errc} / {datetime.datetime.now()}")
            maxRetry += 1
    ...
    while maxRetry < 60:
        try:
            ...
            # Error Occured
                ...
                print(f"masterLeague / {errorTable[masterListRequest.status_code]['message']} / {errorTable[masterListRequest.status_code]['description']} / Try after {backoff}")
                time.sleep(backoff)
            else:
                masterListDto = pd.DataFrame()
                print(f"masterLeague / {errorTable[masterListRequest.status_code]['message']} / {errorTable[masterListRequest.status_code]['description']}")
                break  
        except requests.exceptions.ConnectionError as errc:
            time.sleep(60)
            print(f"masterLeague / {errc} / {datetime.datetime.now()}")
            maxRetry += 1
...
def ibsgpdLeague(apiKey, division, tier, queue="RANKED_SOLO_5x5"):
    # LeagueEntryDto
        ...
        while maxRetry < 60:
            try:
                ...
                # Error Occured
                    ...
                    print(f"ibsgpdLeague / {errorTable[LeagueEntryRequest.status_code]['message']} / {errorTable[LeagueEntryRequest.status_code]['description']} / Try after {backoff}")
                    time.sleep(backoff)
                else:
                    LeagueEntryDtoTemp = pd.DataFrame()
                    print(f"ibsgpdLeague / {errorTable[LeagueEntryRequest.status_code]['message']} / {errorTable[LeagueEntryRequest.status_code]['description']}")
                    break  
            except requests.exceptions.ConnectionError as errc:
                time.sleep(60)
                print(f"ibsgpdLeague / {errc} / {datetime.datetime.now()}")
                maxRetry += 1
    ...
    while maxRetry < 60:
        try:
            ...
            # Error Occured
                ...
                print(f"summonerId / {errorTable[SummonerRequest.status_code]['message']} / {errorTable[SummonerRequest.status_code]['description']} / Try after {backoff}")
                time.sleep(backoff)
            else:
                SummonerDto = pd.DataFrame()
                print(f"summonerId / {errorTable[SummonerRequest.status_code]['message']} / {errorTable[SummonerRequest.status_code]['description']}")
                break  
        except requests.exceptions.ConnectionError as errc:
            time.sleep(60)
            print(f"summonerId / {errc} / {datetime.datetime.now()}")
            maxRetry += 1
            ...
            while maxRetry < 60:
                try:
                    ...
                    if MatchlistRequest.status_code == 200:
                        ...
                        print(f"matchId / {errorTable[MatchlistRequest.status_code]['message']} / {errorTable[MatchlistRequest.status_code]['description']} / Try after {backoff}")
                        time.sleep(backoff)
                    else:
                        MatchlistDtoTemp = pd.DataFrame()
                        MatchReferenceDtoTemp = pd.DataFrame()
                        print(f"matchId / {errorTable[MatchlistRequest.status_code]['message']} / {errorTable[MatchlistRequest.status_code]['description']}")
                        break  
                except requests.exceptions.ConnectionError as errc:
                    time.sleep(60)
                    print(f"matchId / {errc} / {datetime.datetime.now()}")
                    maxRetry += 1
            # 더 이상 추가할 페이지가 없는 경우 종료
    ...
    while maxRetry < 60:
        try:
            ...
            # Error Occured
                ...
                print(f"matchResult / {errorTable[MatchRequest.status_code]['message']} / {errorTable[MatchRequest.status_code]['description']} / Try after {backoff}")
                time.sleep(backoff)
            else:
                ...
                print(f"matchResult / {errorTable[MatchRequest.status_code]['message']} / {errorTable[MatchRequest.status_code]['description']}")
                break  
        except requests.exceptions.ConnectionError as errc:
            time.sleep(60)
            print(f"matchResult / {errc} / {datetime.datetime.now()}")
            maxRetry += 1
    ...
    while maxRetry < 60:
        try:
            ...
            # Error Occured
                ...
                print(f"matchTimeline / {errorTable[MatchTimelineRequest.status_code]['message']} / {errorTable[MatchTimelineRequest.status_code]['description']} / Try after {backoff}")
                time.sleep(backoff)
            else:
                ...
                MatchParticipantFrameDto = pd.DataFrame()
                MatchEventDto = pd.DataFrame()
                print(f"matchTimeline / {errorTable[MatchTimelineRequest.status_code]['message']} / {errorTable[MatchTimelineRequest.status_code]['description']}")
                break  
        except requests.exceptions.ConnectionError as errc:
            time.sleep(60)
            print(f"matchTimeline / {errc} / {datetime.datetime.now()}")
            maxRetry += 1
    ...
    while maxRetry < 60:
        try:
            ...
            # Error Occured
                ...
                print(f"spectator / {errorTable[CurrentGameRequest.status_code]['message']} / {errorTable[CurrentGameRequest.status_code]['description']} / Try after {backoff}")
                time.sleep(backoff)
            else:
                CurrentGameInfo = pd.DataFrame()
                print(f"spectator / {errorTable[CurrentGameRequest.status_code]['message']} / {errorTable[CurrentGameRequest.status_code]['description']}")
                break
        except requests.exceptions.ConnectionError as errc:
            time.sleep(60)
            print(f"spectator / {errc} / {datetime.datetime.now()}")
            maxRetry += 1
